...
def load_cv_from_colvar(
    ...
    *,
    ...
    data = np.loadtxt(colvar_path, dtype=np.float64, comments="#")
    if data.ndim == 1:
        data = data[None, :]
...
def main(cfg: Case1Config) -> FunctionalHierarchicalTensorGaussian:
    ...
    for rd in run_dirs:
        ...
        if not colvar_path.exists():
            raise FileNotFoundError(f"Missing {colvar_path}")
