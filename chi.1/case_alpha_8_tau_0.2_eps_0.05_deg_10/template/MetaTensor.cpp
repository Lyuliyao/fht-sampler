...
void MetaTensor::registerKeywords(Keywords& keys) {
  ...
  keys.add("compulsory","MPI_ROOT_ONLY","1","If 1, evaluate METATENSOR only on rank 0 of each simulation communicator and broadcast results.");
  ...
  PLUMED_BIAS_INIT(ao)
{
  ...
  parse("MPI_ROOT_ONLY", mpi_root_only_int);
  pbc_ = (pbc_int != 0);
  whiten_ = (whiten_int != 0);
  normalize_ = (normalize_int != 0);
  mpi_root_only_ = (mpi_root_only_int != 0);
  is_local_root_ = (!mpi_root_only_) || (comm.Get_rank() == 0);
  checkRead();
  temp=0.0;
  parse("TEMP",temp);
...
void MetaTensor::calculate() {
  ...
  double ene = 0.0;
  double totf2 = 0.0;
  const int nv = getNumberOfArguments();
  std::vector<double> output_forces(nv, 0.0);

  if (is_local_root_) {
    if (static_cast<int>(rhos_.size()) != num_collections) {
      plumed_merror("METATENSOR internal error: number of loaded collections does not match NUMCOLLECTIONS");
    }
    ...
    const double tau = tau_factor * eps;
    ...
    for (int j = 0; j < num_collections; ++j) {
      ...
      rho[j] = rho_raw;
      const double t = (rho_raw - eps) / tau;
      const double rho_eff = eps + tau * softplus(t);
    ...
    for (int i = 0; i < nv; ++i) {
      ...
      double f = 0.0;
      for (int j = 0; j < num_collections; ++j) {
        const double t = (rho[j] - eps) / tau;
        const double sigma = sigmoid(t);
        const double rho_eff = eps + tau * softplus(t);
  ...
  if (mpi_root_only_ && comm.Get_size() > 1) {
    comm.Bcast(ene, 0);
    comm.Bcast(totf2, 0);
    comm.Bcast(output_forces, 0);
  }

  for (int i = 0; i < nv; ++i) {
    setOutputForce(i, output_forces[i]);
  }
